...
public class RandomGenerator {
	...
	private static final String ALPHABETS_WITH_PUNCTUATIONS = "abcdefghijklmnopqrstuvwxyz!,.;:";
	...
	/**
	 * @param characters an array of characters to select from
	...
	/**
	 * @param string the input string from which a character needs to be chosen
	...
	/**
	 * @param wordCount number of words of size 10 needed
	...
	/**
	 * @param stringLengths an array of integers specifying the string lengths
	...
	/**
	 * @param string string from where characters are chosen for the random string
	...
	/**
	 * @param string    string from where the characters will be chosen
	...
	public static List<String> strings(String string, int wordCount) {
		return IntStream.range(0, wordCount)
				.boxed()
				.map(i -> string(string, number(10)))
				.collect(Collectors.toList());
	}

	/**
	 * @param wordCount number of words in the name
	 * @return
	 */
	public static String name(int wordCount) {
		return IntStream.range(0, wordCount)
				.boxed()
				.map(i -> string(number(10)))
				.map(str -> str.toUpperCase()
						.charAt(0) + str.substring(1))
				.reduce("", (a, b) -> a + " " + b);
	}
	...
	public static String paragraph(int wordCount) {
		return IntStream.range(0, wordCount)
				.boxed()
				.map(i -> string(ALPHABETS_WITH_PUNCTUATIONS, 10))
				.reduce("", (a, b) -> a + b);
	}

	/**
	 * @param limit the maximum value the random integer can take (minimum is 0)
	...
	/**
	 * @param minLimit the minimum value the random integer can take
	 * @param maxLimit the maximum value the random integer can take
	 * @return a positive random integer
	 */
	...
	/**
	 * @param limits an array of integer limits
	...
	/**
	 * @param limit the maximum value the random number can take (minimum is 0)
	...
	/**
	 * @param minLimit the minimum value the random number can take
	 * @param maxLimit the maximum value the random number can take
	 * @return a positive random number
	 */
	...
	/**
	 * @param limits an array of limits
	 ...
	 * @param limit maximum value of each integer entry (minimum is 0)
	 ...
	 * @param limit maximum value of each entry (minimum is 0)
